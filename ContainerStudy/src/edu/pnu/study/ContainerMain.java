...
public class ContainerMain {
	...
	public static void main(String[] args) {
		...
		Thread th = new Thread(() -> {
			...
			while(cm.getThreadCheck()) {// 메인스레드 부모스레드가 죽었으면 자기도 루프 끝내라는 코드
				System.out.println("현재스래드 갯 수: "+ moc.getOperatorsize());
				
				// 생존 기준 시간인 millisecond를 넘겨서 사용되지 않은 객체는 제거한다. //10초 생명주기
				moc.releaseOldObject(15000);
				
				try {
					Thread.sleep(1000); // cpu 사용량 많아서 1초 간격으로 슬립시켜준다.
